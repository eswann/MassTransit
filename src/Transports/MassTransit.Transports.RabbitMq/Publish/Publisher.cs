namespace MassTransit.Transports.RabbitMq.Publish
{
    ...
    {
        private readonly ConcurrentQueue<IMessage> _messageBuffer = new ConcurrentQueue<IMessage>();
        ...
        static readonly ILog _logger = Logger.Get<Publisher>();

        public Publisher(Lazy<IServiceBus> serviceBus, IPublishSettings publishSettings, IConfirmer confirmer, 
        ...
        public void Publish<T>(T message, bool forcePublish = false) where T: IMessage
        {
            if (PublicationEnabled || forcePublish)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    OnPublicationFailed();
                    _logger.Error("An error occurred while publishing to MassTransit", ex);
                }
            }
            else
            {
        ...
        private void RepublishStoredMessages()
        {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    ...
                    {
                        IEnumerable<IMessage> storedMessages =
        ...
        private void RepublishStoredMessage()
        {
            ...
            Task.Factory.StartNew(() => 
            {
                try
                {
                    IMessage message = _unconfirmedMessageRepository.GetAndDeleteMessages(_publishSettings.PublisherId, 1).FirstOrDefault();
